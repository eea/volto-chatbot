...
const processTextToSemanticChunks = async (
  ...
  // Step 2: Structure sentences with context
  const structuredSentences = structureSentences(sentences, bufferSize);

  // Step 3: Generate and attach embeddings
  const sentencesWithEmbeddings =
    await generateAndAttachEmbeddings(structuredSentences);

  // Step 4: Calculate cosine distances and identify significant shifts
  const { updatedArray: sentencesWithDistances, significantShiftIndices } =
...
// Define the SentenceObject type
